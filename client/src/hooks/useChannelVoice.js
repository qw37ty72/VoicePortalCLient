...
import { applyRnnoise, applyNoiseGate } from './useNoiseGate';
...
export function useChannelVoice(socket, channelId, enabled, initialMembers = [], localVideoStream = null) {
  ...
  useEffect(() => {
    ...
    function createPeer(remoteSocketId, userId, user) {
    ...
    function doOffer(remoteSocketId, userId, user) {
      const pc = createPeer(remoteSocketId, userId, user);
      if (!pc) return;
    ...
    socket.on('webrtc-offer', async ({ from, userId: uid, user: u, offer, room, channelId: chId }) => {
      ...
      if (!localStreamRef.current) {
        pendingOffers.push({ from, userId: uid, user: u, offer });
        return;
      }
      let p = peers.get(from);
      if (!p) {
        createPeer(from, uid, u);
        p = peers.get(from);
      }
      const pc = p?.pc;
      if (!pc) return;
    ...
    async function startVoice() {
      try {
        ...
        try {
          myStream = (await applyRnnoise(rawMicStream)) ?? rawMicStream;
          myStream = await applyNoiseGate(myStream, 0.13);
        } catch (e) {
          myStream = rawMicStream;
        }
        if (!myStream?.getAudioTracks?.().length) {
          console.error('[useChannelVoice] Нет аудиотрека');
        ...
        while (pendingOffers.length > 0) {
          const { from, userId: uid, user: u, offer } = pendingOffers.shift();
          let p = peers.get(from);
          if (!p) {
            createPeer(from, uid, u);
            p = peers.get(from);
          ...
          try {
            ...
            console.error('[useChannelVoice] pending offer failed', err);
          }
        }
      } catch (err) {
    ...
    return () => {
      ...
      peers.clear();
      pendingOffersRef.current.length = 0;
      setRemotePeers([]);
